...
export function random_pair(current_rankings: ItemRanking[], items_to_image_url: ItemsToImageUrl=ITEMS_TO_IMAGE_URLS): [string, string] {
    ...
    if (unranked_items.length > 0) {
        ...
        const item_b = unranked_items[Math.floor(Math.random() * unranked_items.length)];
        return [item_a, item_b];
    }
    
    const low_comparison_percentile = 0.1;
    const comparison_percentiles = current_rankings.map(ranking => ranking.comparison_count / current_rankings.reduce((acc, r) => acc + r.comparison_count, 0));
    ...
    if (low_comparison_items.length > 0) {
        ...
        const item_b = low_comparison_items[Math.floor(Math.random() * low_comparison_items.length)];
        return [item_a, item_b];
    }

    let elo_range = 100;
    const elo_range_expansion_factor = 1.5; // Factor to increase the elo range if no suitable pair is found
    const item_a = items[Math.floor(Math.random() * items.length)];
    let item_b = null;
    const sorted_rankings = current_rankings.sort((a, b) => a.elo_ranking - b.elo_ranking);
    while(item_b === null) {
        // Find all items within the elo range of item_a, except itself obviously
        const candidate_items = sorted_rankings.filter(ranking =>
            ranking.item_name !== item_a &&
            Math.abs(ranking.elo_ranking - (sorted_rankings.find(r => r.item_name === item_a)?.elo_ranking || 0)) <= elo_range
        ).map(r => r.item_name);

        if (candidate_items.length > 0) {
            item_b = candidate_items[Math.floor(Math.random() * candidate_items.length)];
        } else {
            elo_range *= elo_range_expansion_factor; // Expand the elo range if no suitable pair is found
            if (elo_range > 1000) {
                // If the elo range gets too large, just pick a random item
                item_b = items[Math.floor(Math.random() * items.length)];
